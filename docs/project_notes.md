...
- AudioDeviceCmdlets PowerShell module installed (CurrentUser scope) — use for reliable audio device switching
...
| `start_receiver.ps1` | Kills stale instance, starts `pythonw mic_receiver.py` hidden, saves PID to `receiver.pid` |
| `stop_receiver.ps1` | Kills receiver by PID file; fallback WMI scan |
